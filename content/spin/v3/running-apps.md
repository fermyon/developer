...
This supports splitting an application across environments, while still distributing it as a single unit. For example, suppose your application contains the following layers:
...
* An AI service which requires a GPU or other specialized (and costly) hardware

You could split the application across the tiers by using `--component-id`. For example:
...
> In practice you'd set these commands up in a scheduler or orchestrator rather than typing them interactively.

If you run a subset which includes a component that uses [local service chaining](./http-outbound#local-service-chaining), then you must also include all chaining targets in the subset.  You cannot use [self-requests](./http-outbound#making-http-requests-within-an-application) in a split application.
