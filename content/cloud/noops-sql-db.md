...
When developing locally, you may also want to test queries, check tables were created properly, or inspect the data in your local SQLite database. [`sqlite3`](https://sqlite.org/cli.html) is a command line program that allows users to evaluate queries interactively. Simply use the `sqlite3` CLI and point it to the `.spin/sqlite_db.db` file located in your application directory. _Note: `.spin/sqlite_db.db` is created on `spin up`._
